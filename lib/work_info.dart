...
class _WorkInfoPageState extends State<WorkInfoPage> {
  ...
  Future<void> _fetchWorkEntries() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
            'http://192.168.1.9:4000/api/getCVWork?accountID=$accountID'),
  ...
  Future<void> _saveWorkEntries() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('http://192.168.1.9:4000/api/saveCVWork'),
        headers: {'Content-Type': 'application/json'},
        body: body,
      );
  ...
  Future<void> _deleteWorkExperienceEntry(int index) async {
    ...
    if (WorkExpID != null) {
      try {
        final response = await http.post(
          Uri.parse('http://192.168.1.9:4000/api/deleteCVWork'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'WorkExpID': WorkExpID}),
        );

        final response2 = await http.post(
          Uri.parse('http://192.168.1.9:3011/api/deleteCVWork'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode({'WorkExpID': WorkExpID}),
        );
        if (!mounted) return;
